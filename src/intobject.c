...
static void intobject_op_print (object_t *obj);
...
/* Print. */
static void
intobject_op_print (object_t *obj)
{
	printf ("%d", intobject_get_value (obj));
}
...
intobject_load_buf (const char **buf, size_t *len)
{
	...
	if (*len < sizeof (int)) {
		error ("failed to load int buffer.");
