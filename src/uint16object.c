...
static void uint16object_op_print (object_t *obj);
...
/* Print. */
static void
uint16object_op_print (object_t *obj)
{
	printf ("%" PRIu16, uint16object_get_value (obj));
}
...
uint16object_op_dump (object_t *obj)
{
	...
	snprintf (buf, DUMP_BUF_SIZE, "<uint16 %" PRIu16 ">", uint16object_get_value (obj));
...
uint16object_load_buf (const char **buf, size_t *len)
{
	...
	if (*len < sizeof (uint16_t)) {
		error ("failed to load uint16 buffer.");
...
uint16object_new (uint16_t val, void *udata)
{
	...
	OBJECT_NEW_INIT (obj, OBJECT_TYPE_UINT16, udata);
